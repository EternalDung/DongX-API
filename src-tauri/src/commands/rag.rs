...
/// 索引状态类型：实现在 [`crate::rag::index`]（管理面命令与 MCP 工具共用），
/// 此处重导出以保持既有引用路径（含 ts-rs 导出）不变。
pub use crate::rag::index::IndexStatus;
...
pub async fn get_index_status(
    ...
    crate::rag::index::compute_index_status(&state.db, &kb_id).await
}

/// 重建索引：按知识库「当前」嵌入模型，重新向量化全部分块并写回。
///
/// 实现见 [`crate::rag::index::reindex_kb`]（管理面命令与 MCP 工具共用同一份）。
#[tauri::command]
pub async fn reindex_kb(
    state: State<'_, Arc<AppState>>,
    kb_id: String,
) -> Result<IndexStatus, String> {
    crate::rag::index::reindex_kb(&state.db, &kb_id).await
}
